...
check was introduced: **41 findings across 17 files
and 2,192 lines**. CI compares against that budget and fails only when a
...
|---|---:|---|---|---|
| `RS003` | 18 | error | typed errors | panic-based control flow (`unwrap`/`expect`/`panic!`) |
| `RS001` | 9 | warn | immutable values | mutable local binding (`let mut`) |
| `RS007` | 7 | warn | effects pushed outward | direct stdout/stderr effect in library code |
| `RS009` | 5 | error | illegal states excluded by types | `unsafe` block |
...
*typed errors* · 18 occurrences at baseline
...
*effects pushed outward* · 7 occurrences at baseline

Emit through the ores-otel tracing layer so the effect lives at the outward edge and stays observable.

### `RS009` — `unsafe` block

*illegal states excluded by types* · 5 occurrences at baseline
